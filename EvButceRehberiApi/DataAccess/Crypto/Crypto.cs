...
namespace DataAccess.Crypto
{
    public class Sifrele
    {
        ...
        {
            byte[] data = Encoding.Default.GetBytes(parola);
        ...
        public static string Descrypt(string parola)
        {
            byte[] data = Convert.FromBase64String(parola);
            using (MD5CryptoServiceProvider md5 = new MD5CryptoServiceProvider())
            {
                byte[] keys = md5.ComputeHash(Encoding.Default.GetBytes(hash));
                using (TripleDESCryptoServiceProvider tripleDES = new TripleDESCryptoServiceProvider()
